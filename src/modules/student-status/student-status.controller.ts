...
export class StudentStatusController {
  ...
  async changeStatus(
    ...
    return await this.studentStatusService.changeStatus(student_id, dto, req.user?.user_id);
  }
  ...
  async getHistory(@Query('page') page = 1, @Query('limit') limit = 10, @Query('student_id') student_id: string) {
  ...
  async getStatusCounts(@Query('start_date') start_date: string, @Query('end_date') end_date: string) {
