...
import { LEVEL_TARGETS, KPI_SCORES, KpiTableResponse, SellerKpiTableRow, DayKpiCell, WeekGroup } from './kpi.constanta'
...
export class KpiTableService {
    ...
    async getKpiTable(year: number, month: number): Promise<KpiTableResponse> {
        ...
        const sellerRows = await Promise.all(
            sellers.map((seller) =>
                this.buildSellerRow(seller, year, month, today),
            ),
        );
    ...
    async getSellerKpiTable(
        seller_id: string,
        year: number,
        month: number,
    ): Promise<SellerKpiTableRow> {
        const sellers = await this.tableRepo.getSellers();
    ...
    private async buildSellerRow(
        ...
        if (dailyKpis.length > 0) {
            ...
            console.log(`[KPI Table] Last KPI date:`, new Date(dailyKpis[dailyKpis.length-1].date).toISOString().slice(0,10));
        }
        ...
        for (let d = 1; d <= daysInMonth; d++) {
            ...
            const conversion_pct =
                leadsCount > 0 && !isFuture
                    ? parseFloat(((salesCount / leadsCount) * 100).toFixed(1))
                    : null;

            // Kunlik KPI ball hisoblash (0–100%)
            const daily_kpi_pct = isFuture || !kpi
                ? null
                : this.calcDailyKpiPct(kpi, conversion_pct, saleSumToday, targets);

            // Plan vs Fakt: targetga nisbatan qancha bajarildi (calls % × 0.4 + talk % × 0.6)
            const plan_vs_fact_pct = isFuture || !kpi
                ? null
                : this.calcPlanVsFact(kpi, saleSumToday, targets);
            ...
            cells.push({
                ...
                talk_time_display: isFuture || kpi?.talk_time_seconds == null
                    ? null
                    : this.formatSeconds(kpi.talk_time_seconds),
                ...
                sale_sum_display:  isFuture || saleSumToday == null
                    ? null
                    : this.formatSum(saleSumToday),
        ...
        const avg_kpi          = kpi_values.length
            ? Math.round(kpi_values.reduce((a, b) => a + b, 0) / kpi_values.length)
            : 0;
    ...
    // ─── Plan vs Fakt % ─────────────────────────────────────────
    private calcPlanVsFact(
        kpi: any,
        sale_sum: number | null,
        targets: typeof LEVEL_TARGETS['junior'],
    ): number {
        const callRatio = kpi.calls_target > 0
            ? Math.min((kpi.calls_count ?? 0) / kpi.calls_target, 1)
            : 0;
        const talkRatio = kpi.talk_time_target > 0
            ? Math.min((kpi.talk_time_seconds ?? 0) / kpi.talk_time_target, 1)
            : 0;

        const daily_sale_target = Number(targets.sale_sum_target) / 22;
        const saleRatio = daily_sale_target > 0
            ? Math.min((sale_sum ?? 0) / daily_sale_target, 1)
            : 0;

        // Og'irlik: calls 30%, talk_time 30%, sotuv 40%
        return Math.round((callRatio * 0.30 + talkRatio * 0.30 + saleRatio * 0.40) * 100);
    }
