...
export class SellerKpiService {
    ...
    private async updateConversion(
        ...
        await Promise.all(sellers.map(async (seller) => {
            ...
            const { leads_count, sales_count } =
                await this.monthlyRepo.getConversionData(seller.id, fromDate, toDate);

            const conversion_pct =
                leads_count > 0
                    ? parseFloat(((sales_count / leads_count) * 100).toFixed(2))
                    : 0;
    ...
    private async updateMonthlySaleSum(year: number, month: number) {
        ...
        await Promise.all(sellers.map(async (seller) => {
            ...
            const { total_sum, total_count } =
                await this.monthlyRepo.getSellerMonthlySale(seller.id, year, month);
    ...
    async calculateSellerKpiScore(
        ...
        const ratio = (done: number) =>
            dailyKpis.length > 0 ? done / dailyKpis.length : 0;
    ...
    async calculateAllSellersKpiScore(year?: number, month?: number) {
        const sellers = await this.dailyRepo.getSellers();
        return Promise.all(
            sellers.map((s) => this.calculateSellerKpiScore(s.id, year, month)),
        );
    }
    ...
    private calcSaleScore(sale_sum: bigint, sale_target: bigint): number {
        ...
        if (pct >= 50) return Math.round(KPI_SCORES.SALE_SUM * 0.50); // 12-13
