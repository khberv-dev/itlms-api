...
export class KpiTableController {
   ...
   */
  @Get()
  async getTable(@Query('year') year?: string, @Query('month') month?: string) {
    const now = new Date();
    return this.kpiTableService.getKpiTable(year ? +year : now.getFullYear(), month ? +month : now.getMonth() + 1);
  }
