...
import { ApiForbiddenResponse, ApiNoContentResponse, ApiTags } from '@nestjs/swagger';
...
export class AuthController {
  ...
  @Post('/login')
  @HttpCode(200)
  async login(@Req() { user }: { user }, @Res({ passthrough: true }) response: express.Response, @Body() _: LoginDto) {
  ...
  async refresh(@Req() { user }: { user }, @Res({ passthrough: true }) response: express.Response) {
    const accessToken = this.authService.getJWT('access', user.id, user.role);
    const refreshToken = this.authService.getJWT('refresh', user.id, user.role);
