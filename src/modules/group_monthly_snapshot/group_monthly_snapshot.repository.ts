...
export class GroupSnapshotRepository {
  ...
  async getSnapshotsByMentor(mentor_id: string, years_months: Array<{ year: number; month: number }>) {
    return this.prisma.group_monthly_snapshot.findMany({
      where: {
        OR: [{ mentor_id }, { assistant_id: mentor_id }],
        AND:
          years_months.map(({ year, month }) => ({ year, month })).length > 0
  ...
  async getSnapshotsForCompany(years_months: Array<{ year: number; month: number }>) {
  ...
  async getSnapshotsForGroup(group_id: string, years_months: Array<{ year: number; month: number }>) {
    if (years_months.length === 0) return [];
    return this.prisma.group_monthly_snapshot.findMany({
      where: {
        group_id,
  ...
  async getGroupIds(filter: { group_id?: string; mentor_id?: string }): Promise<string[]> {
    ...
    if (filter.mentor_id) {
      const groups = await this.prisma.group.findMany({
        where: {
          OR: [{ mentor_id: filter.mentor_id }, { assistant_id: filter.mentor_id }],
        },
        select: { id: true },
      });
  ...
  emptyStats() {
    return {
      active_count_start: 0,
      churned_count: 0,
      churn_rate: 0,
      company_churn_rate: 0,
      mentor_churn_rate: 0,
      retention_rate: 0,
      ltv: 0,
      is_realtime: false,
  ...
  buildYearMonthRange(startYear: number, startMonth: number, endYear: number, endMonth: number) {
