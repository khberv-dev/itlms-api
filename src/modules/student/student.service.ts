...
export class StudentService {
    ...
    async getStudentsForAdmin(filter: GetStudentFilterDto, page: number, limit: number) {
        const where = buildStudentWhere(filter);
        const { items, meta } = await this.studentRepository.getStudentsForAdmin(
            where,
            page,
            limit,
        );
        ...
        const mappedItems = items.map((student: any) => {
            ...
            const submissionScore = student.assignment_submissions.reduce(
    ...
    async getStudentsExpiry(page: number, limit: number, filter?: 'active' | 'expired' | '7') {
        ...
        const mappedItems = students.items.map((student: any, index: number) => {
            ...
            const endDate = student.access_expires_at
            ...
            const status =
                endDate && endDate.getTime() < today.getTime() ? 'Expired' : 'Active';
    ...
    async getAdminDashboardStats() {
        const [total_students, active_groups, total_teachers, blocked_users] =
            await Promise.all([
    ...
    async getRankings(studentId: string) {
        ...
        const groupStudents =
            await this.studentRepository.getGroupStudentsWithPoints(
                student.group_id,
            );
    ...
    async getStats(studentId: string) {
        ...
        const data =
            await this.studentRepository.getStudentAttendancesAndSubmissions(
                studentId,
                student.group_id,
            );
        ...
        const group_rank =
            allPoints.findIndex((s) => s.id === studentId) + 1;
    ...
    async create(data: CreateStudentDto) {
        ...
        const new_user = await this.userService.create({ ...user, role: Role.student });
        const new_student =  await this.studentRepository.create({ ...student, user_id: new_user.id });
    ...
    async update(data: UpdateStudentDto, id: string, file: Express.Multer.File) {
        ...
        if (file) {
            const file_res = await this.fileService.upload(file, { is_active: true, folder: 'student-avatars' }, student.user?.avatar_id);
        ...
        await Promise.all([
            ...
            this.userService.update(student.user_id, user_data),

            Object.keys(student_data).length &&
    ...
    async extendAccess(student_id: string, months: number): Promise<void> {
        ...
        if (currentExpiry && currentExpiry > now) {
            ...
            await this.studentStatusService.changeStatus(student_id, {to_status: StudentStatus.active})
