...
export class MentorService {
    ...
    async findAll() {
        ...
        return mentors.map((mentor: any) => {
            ...
            const fullName =
                `${mentor.user?.first_name ?? ''} ${mentor.user?.last_name ?? ''}`.trim();
    ...
    async update(data: UpdateMentorDto, id: string) {
        ...
        await Promise.all([
            ...
            this.userService.update(mentor.user_id, user_data),

            Object.keys(mentor_data).length &&
            this.mentorRepository.update(id, mentor_data),
        ]);
