...
export class SellerService {
  ...
  async update(data: UpdateSellerDto, id: string) {
    ...
    await Promise.all([
      Object.keys(user_data).length && this.userService.update(seller.user_id, user_data),

      Object.keys(seller_data).length && this.sellerRepository.update(id, seller_data),
    ]);
