...
export class SellerStatisticService {
  constructor(private readonly sellerStatisticRepository: SellerStatisticRepository) {}

  async getKpiCards(sellerId: string, start: Date, end: Date) {
    const salesAgg = await this.sellerStatisticRepository.getSalesAggregateByPeriod(sellerId, start, end);

    const statsAgg = await this.sellerStatisticRepository.getDailyStatsAggregateByPeriod(sellerId, start, end);
  ...
  async getDailyTalkAndContacts(sellerId: string, start: Date, end: Date) {
    const data = await this.sellerStatisticRepository.getDailyTrendByPeriod(sellerId, start, end);
  ...
  async compareWithLastPeriod(sellerId: string, start: Date, end: Date) {
    ...
    const currentSales = await this.sellerStatisticRepository.getSalesAggregateByPeriod(sellerId, start, end);

    const currentStats = await this.sellerStatisticRepository.getDailyStatsAggregateByPeriod(sellerId, start, end);

    // ---- 3) Oldingi davr uchun ma’lumot ----
    const prevSales = await this.sellerStatisticRepository.getSalesAggregateByPeriod(sellerId, prevStart, prevEnd);

    const prevStats = await this.sellerStatisticRepository.getDailyStatsAggregateByPeriod(sellerId, prevStart, prevEnd);
