...
export class SellerMonthPlanService {
  constructor(private readonly sellerMonthPlanRepository: SellerMonthPlanRepository) {}
  ...
  async setMonthlyPlan(start_date: Date, end_date: Date, sum: number, seller_id: string) {
    ...
    const { baseAmount, remainder } = this.calculateMonthlyAmount(sum, months.length);

    return await this.sellerMonthPlanRepository.setPlanByDateRange(months, sum, seller_id, baseAmount, remainder);
  }
