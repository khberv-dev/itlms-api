...
export class SellerDailyStatisticService {
  constructor(private readonly sellerDailyStatisticRepository: SellerDailyStatisticRepository) {}
  ...
  async syncTodayStatistics() {
    ...
    await Promise.all(
      sellers.map(async (seller) => {
        ...
        const leads_count = await calculateSellerLeadsCount(today, seller.amocrm_id);
  ...
  private aggregateBySip(calls: any[]) {
    ...
    for (const call of calls) {
      const sip = call.accountcode === 'outbound' ? call.caller_id_number : call.destination_number;
