...
export class SaleStatisticService {
    constructor(
        private readonly saleStatisticRepository: SaleStatisticRepository,
    ) { }
    ...
    async getYearlyDynamics(year: number) {
        ...
        // Yillik reja
        const yearPlan = plans.reduce(
            (acc, p) => acc + Number(p.plan || 0),
            0
        );

        // O‘tgan yil bilan solishtirish
        const lastYearSales: any =
            await this.saleStatisticRepository.getYearlySales(year - 1);
        ...
        const growthPercent =
            lastYearTotal > 0
                ? ((totalSales - lastYearTotal) / lastYearTotal) * 100
                : 0;

        // Reja bajarilishi %
        const planCompletionPercent =
            yearPlan > 0 ? (totalSales / yearPlan) * 100 : 0;
    ...
    async getMonthlyConversion(start: string, end: string) {
        ...
        return this.saleStatisticRepository.getMonthlyConversion(
            startDate,
            endDate,
        );
    }
    ...
    async getSalesByAddress(start: string, end: string) {
        return this.saleStatisticRepository.getSalesByAddress(
            new Date(start),
            new Date(end),
        );
    }
    ...
    async getSalesByJob(start: string, end: string) {
        return this.saleStatisticRepository.getSalesByJob(
            new Date(start),
            new Date(end),
        );
    }
    ...
    async getSellersByAverageCheck(start: string, end: string) {
        ...
        const data =
            await this.saleStatisticRepository.getSellersByAverageCheck(
                startDate,
                endDate,
            );
